(function () {
    ...
    // Example of loading additional packs if needed
    const iconify_devicon = fetch('https://unpkg.com/@iconify-json/devicon@1.2.44/icons.json').then((res) => res.json())
        .catch(e => console.error("Failed to fetch devicon icon pack:", e));



    // Intercept Mermaid loading into the global scope so we can register
    // the icon packs before Mermaid is initialized by Material for MkDocs
        ...
        set: async function (value) {
            ...
            try {
                value.registerIconPacks([
                    ...
                    { name: 'devicon', loader: () => iconify_devicon },
                ]);
            } catch (e) {
